...
macro_rules! serve_static_file {
    ($file:expr) => {{
        ...
        if path.exists() && path.is_file() {
            web::resource(concat!("static/", $file)).route(web::get().to(|| async move {
                let path = std::path::Path::new("static").join($file);
            ...
            let contents = Arc::new(include_str!(concat!("../static/", $file)).to_string());
            ...
            web::resource(concat!("static/", $file)).route(web::get().to(
